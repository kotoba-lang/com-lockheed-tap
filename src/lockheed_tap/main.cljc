(ns lockheed-tap.main
  "Kotodama WASM entrypoint for the Lockheed_tap clean-room actor (L4) — Clojure port.
  ...
  facts (`lockheed_tap.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "lockheed_tap")
(def tier "L4")
...
(def entity-specs
  [{:entity "Case"      :plural "cases"      :id-prefix "lockheed_cas"
    :fields [:number :jurisdiction :status :filedAt] :required [:number :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"    :id-prefix "lockheed_par"
    :fields [:caseId :role :name]            :required [:role :name]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Document"  :plural "documents"  :id-prefix "lockheed_doc"
    :fields [:caseId :type :contentRef]      :required [:type]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Incident"  :plural "incidents"  :id-prefix "lockheed_inc"
    :fields [:type :jurisdiction :occurredAt] :required [:type :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Watchlist" :plural "watchlists" :id-prefix "lockheed_wat"
    :fields [:name :program :entries]        :required [:name :program]
    :coerce {:entries :int}          :refs {}}
   {:entity "Sanction"  :plural "sanctions"  :id-prefix "lockheed_san"
...
(defn emit-facts
  "EAVT facts for one record: {\"lockheed_tap.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "lockheed_tap-compat" :tier tier :entities entities} 200])
...
(defn start! [] :lockheed_tap-compat/ready)
